...
from tests.integration.db_helpers import set_up_survey, set_up_collection_exercise, set_up_job
...
def test_processing_jobs(run_test_app):
    ...
    with Session(engine) as session:
        ...
        survey = set_up_survey(uuid.uuid4(), name="test survey", metadata={"key": "value"},
                      sample_validation_rules=sample_validation_rules, session=session)
    ...
    assert jobs[0].staging_row_number == 5
